...
function getSequenceInfo() {
    try {
        ...
        try {
            // Get in point directly - returns raw number value
            var inPoint = sequence.getInPoint();
            
            // 🔬 DEBUG: Log raw Premiere data before processing
            var debugRawData = {
                sequenceName: sequence.name,
                rawInPoint: inPoint,
                rawOutPoint: null, // Will be set below
                inPointType: typeof inPoint,
                inPointString: String(inPoint),
                hasGetInPointMethod: typeof sequence.getInPoint === 'function'
            };
            
            // ExtendScript returns string representation of numbers, so parse it
            var inPointSeconds = parseFloat(inPoint);
            var hasInPoint = (!isNaN(inPointSeconds) && inPointSeconds >= 0);
            
            // 🔍 DEBUG: Log parsing analysis
            var debugParsingData = {
                rawInPoint: inPoint,
                parsedInPointSeconds: inPointSeconds,
                isInPointNaN: isNaN(inPointSeconds),
                isInPointFinite: isFinite(inPointSeconds),
                inPointPassesValidation: (!isNaN(inPointSeconds) && inPointSeconds >= 0)
            };
        ...
        try {
            // Get out point directly - returns raw number value
            var outPoint = sequence.getOutPoint();
            
            // 🔬 DEBUG: Update raw data with out point info
            debugRawData.rawOutPoint = outPoint;
            debugRawData.outPointType = typeof outPoint;
            debugRawData.outPointString = String(outPoint);
            debugRawData.hasGetOutPointMethod = typeof sequence.getOutPoint === 'function';
            
            // ExtendScript returns string representation of numbers, so parse it
            var outPointSeconds = parseFloat(outPoint);
            var hasOutPoint = (!isNaN(outPointSeconds) && outPointSeconds >= 0);
            
            // 🔍 DEBUG: Update parsing analysis with out point
            debugParsingData.rawOutPoint = outPoint;
            debugParsingData.parsedOutPointSeconds = outPointSeconds;
            debugParsingData.isOutPointNaN = isNaN(outPointSeconds);
            debugParsingData.isOutPointFinite = isFinite(outPointSeconds);
            debugParsingData.outPointPassesValidation = (!isNaN(outPointSeconds) && outPointSeconds >= 0);
        ...
        // Debug info for troubleshooting (enhanced with detailed logging)
        result.debug = {
...
// Function to import and place audio at a specific time position (for in/out point placement)
function importAndPlaceAudioAtTime(filePath, trackIndex, timeSeconds) {
    var result = {
        success: false,
        step: "initialization",
        error: "",
    ...
    try {
        ...
        // Step 4: Create time object for specific placement using proper Premiere Pro API
        result.step = "creating_time_object";
        var placementTime;
        
        try {
            // Method 1: Try using sequence.getPlayerPosition() as template and modify it
            var currentTime = sequence.getPlayerPosition();
            placementTime = currentTime; // Use existing time object as template
            placementTime.seconds = timeSeconds;
            
            // If the time object has ticks property, calculate it properly
            if (typeof placementTime.ticks !== 'undefined' && sequence.timebase) {
                placementTime.ticks = Math.floor(timeSeconds * sequence.timebase);
            }
            
            result.debug.timeObjectMethod = "modified_player_position";
        } catch (timeError1) {
            try {
                // Method 2: Try manual time object creation with proper structure
                var timebase = sequence.timebase || 25; // Default to 25fps
                placementTime = {
                    seconds: timeSeconds,
                    ticks: Math.floor(timeSeconds * timebase)
                };
                
                result.debug.timeObjectMethod = "manual_creation";
            } catch (timeError2) {
                // Method 3: Fallback to just using the number directly
                placementTime = timeSeconds;
                result.debug.timeObjectMethod = "number_fallback";
            }
        }
        ...
        // Step 5: Import file (similar to regular import function)
        result.step = "importing_file";
        result.debug.filePath = filePath;
        ...
        // Step 7: Get target track
        result.step = "getting_target_track";
        var finalTrackIndex = parseInt(trackIndex) || 0;
        
        if (finalTrackIndex >= sequence.audioTracks.numTracks) {
            finalTrackIndex = sequence.audioTracks.numTracks - 1;
        }
        
        var targetTrack = sequence.audioTracks[finalTrackIndex];
        result.debug.targetTrackIndex = finalTrackIndex;
        result.debug.totalAudioTracks = sequence.audioTracks.numTracks;
        
        // Step 8: Place at specific time using insertClip
        result.step = "placing_at_specific_time";
        try {
            targetTrack.insertClip(importedItem, placementTime);
        ...
        result.success = true;
        result.step = "completed";
        result.message = "Audio placed at " + formatTime(timeSeconds) + " on track " + (finalTrackIndex + 1);
        result.fileName = importedItem.name;
        result.trackIndex = finalTrackIndex;
...
// Enhanced function: Import and place audio at specific time with smart track placement
function importAndPlaceAudioAtTimeWithSmartTracks(filePath, startingTrackIndex, timeSeconds) {
    var result = {
        success: false,
        step: "initialization",
        error: "",
    ...
    try {
        ...
        // Step 5: Create time object for specific placement using proper Premiere Pro API
        result.step = "creating_time_object";
        var placementTime;
        ...
        try {
            ...
            result.debug.timeObjectMethod = "modified_player_position";
            result.debug.originalTimeObject = {
                seconds: currentTime.seconds,
                ticks: currentTime.ticks || 'undefined'
            };
        } catch (timeError1) {
            try {
                // Method 2: Try manual time object creation with proper structure
                ...
                result.debug.timeObjectMethod = "manual_creation";
                result.debug.timebase = timebase;
            } catch (timeError2) {
                // Method 3: Fallback to just using the number directly
                placementTime = timeSeconds;
                result.debug.timeObjectMethod = "number_fallback";
                result.debug.timeError1 = timeError1.toString();
                result.debug.timeError2 = timeError2.toString();
            }
        }
        
        result.debug.placementTimeSeconds = timeSeconds;
        result.debug.placementTimeFormatted = formatTime(timeSeconds);
        result.debug.finalTimeObject = placementTime;
        ...
        // Step 8: Smart timeline placement with collision detection AT SPECIFIC TIME
        result.step = "smart_timeline_placement_at_time";
        var startingTrackIdx = parseInt(startingTrackIndex) || 0;
        var finalTrackIndex = -1;
        var placementAttempts = [];
        
        // Function to check if track has audio at specific time (modified for in-point placement)
        function hasAudioAtTimePosition(track, timeObj) {
            try {
                if (!track.clips || track.clips.numItems === 0) {
                    return false; // No clips = no conflict
                }
                ...
                for (var i = 0; i < track.clips.numItems; i++) {
                    ...
                    if (timeObj.seconds >= (clipStart - 0.1) && timeObj.seconds <= (clipEnd + 0.1)) {
                        return true;
                    }
                }
                return false;
        ...
        for (var trackIdx = startingTrackIdx; trackIdx < sequence.audioTracks.numTracks; trackIdx++) {
            var track = sequence.audioTracks[trackIdx];
            var hasConflict = hasAudioAtTimePosition(track, placementTime);
            ...
            if (!hasConflict) {
                ...
                foundAvailableTrack = true;
                result.debug.foundAvailableTrack = trackIdx;
                result.debug.foundAt = "time position " + timeSeconds + "s";
                break;
            }
        }
        
        // If no available track found, create new audio track using QE API
        if (!foundAvailableTrack) {
            result.debug.beforeTrackCreation = sequence.audioTracks.numTracks;
            result.debug.attemptingNewTrack = true;
            result.debug.reason = "No available tracks at time " + timeSeconds + "s";
            ...
            try {
                ...
                // Use QE API to add tracks (like existing logic)
                if (result.debug.qeEnabled) {
                    try {
                        var qeSequence = qe.project.getActiveSequence();
                ...
                if (!newTrack) {
                    // Fallback: use existing tracks
                    result.debug.trackCreationFailed = true;
                    result.debug.fallbackStrategy = "Using last available track despite conflicts";
                    finalTrackIndex = sequence.audioTracks.numTracks - 1;
                    foundAvailableTrack = true;
                    result.debug.usingFallbackTrack = finalTrackIndex;
                    result.debug.createdNewTrack = false;
        ...
        // Step 9: Place audio at specific time on selected track
        result.step = "placing_audio_at_specific_time";
        var targetTrack = sequence.audioTracks[finalTrackIndex];
        result.debug.targetTrackExists = (targetTrack !== null);
        ...
        try {
            ...
            result.error = "Failed to insert clip at time " + timeSeconds + "s on track " + finalTrackIndex + ": " + insertError.toString();
            return JSON.stringify(result);
        }
        ...
        if (result.debug.createdNewTrack) {
            message += " (new track created for clean placement)";
        } else if (finalTrackIndex !== startingTrackIdx) {
            message += " (avoided conflicts, moved from track " + (startingTrackIdx + 1) + ")";
        } else {
            message += " (track was available)";
        }
        
        if (result.debug.trackCreationFailed || result.debug.usingCatchFallback) {
            message += " - WARNING: Track creation not supported, may overlap existing audio";
        }
...
// Scan AI SFX bin for existing numbered files with matching prompt
function scanAiSfxBinForNumbers(cleanPrompt) {
    try {
        if (!app || !app.project) {
            return JSON.stringify({
                success: false,
        ...
        var result = {
            ...
            debug: {
                ...
                searchPattern: cleanPrompt,
                filesFound: [],
                aiSfxBinExists: false,
                totalItemsInBin: 0
            }
        };
        ...
        for (var i = 0; i < rootItem.children.numItems; i++) {
            ...
            if (child.type === ProjectItemType.BIN && child.name === "AI SFX") {
                aiSfxBin = child;
                result.debug.aiSfxBinExists = true;
                break;
            }
        }
        
        if (!aiSfxBin) {
            // No AI SFX bin exists yet, so no existing files
            result.success = true;
            result.numbers = [];
            result.debug.aiSfxBinExists = false;
            return JSON.stringify(result);
        }
        
        result.debug.totalItemsInBin = aiSfxBin.children.numItems;
        
        // Scan through all items in the AI SFX bin
        for (var j = 0; j < aiSfxBin.children.numItems; j++) {
            var item = aiSfxBin.children[j];
            
            // Only look at audio files (not sub-bins)
            if (item.type !== ProjectItemType.BIN && item.name) {
                result.debug.filesFound.push(item.name);
                
                // Check if filename matches our pattern: number_cleanPrompt.extension
                // Pattern: starts with digits, underscore, then our clean prompt
                var pattern = new RegExp("^(\\d+)_" + cleanPrompt + "\\.", "i");
                ...
                if (match) {
                    ...
                    if (!isNaN(number)) {
                        ...
                        result.debug.matchedFiles = result.debug.matchedFiles || [];
                        result.debug.matchedFiles.push({
                            filename: item.name,
        ...
        result.success = true;
        result.debug.totalNumbersFound = result.numbers.length;
        result.debug.highestNumber = result.numbers.length > 0 ? Math.max.apply(Math, result.numbers) : 0;
        ...
        return JSON.stringify({
            success: false,
            error: "Exception scanning AI SFX bin: " + error.toString(),
            numbers: []
        });
    }
}
