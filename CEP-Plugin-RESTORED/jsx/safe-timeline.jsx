...
function safeImportAndPlace(filePath, trackIndex) {
    ...
    try {
        ...
        // Find the imported item with multiple strategies for new naming format
        for (var i = 0; i < rootItem.children.numItems; i++) {
            var item = rootItem.children[i];
            
            // Strategy 1: Exact match
            if (item.name === fileName || item.name === baseName) {
                importedItem = item;
                break;
            }
            
            // Strategy 2: Handle new format "N prompt.mp3" - extract just the prompt part
            var spaceFormatMatch = baseName.match(/^(\d+) (.+)$/);
            if (spaceFormatMatch && item.name.indexOf(spaceFormatMatch[2]) !== -1) {
                importedItem = item;
                break;
            }
            
            // Strategy 3: General partial match
