...
class PluginRestarter:
    ...
    def __init__(self, debug_port: int, plugin_name: str):
        self.debug_port = debug_port
        self.plugin_name = plugin_name
        self.debug_url = f"http://localhost:{debug_port}"
    
    def soft_restart(self) -> bool:
        """Try to reload plugin via Chrome DevTools protocol"""
        try:
            import urllib.request
            import json
            
            # Method 1: Try simple page reload via eval
            try:
                ...
                if targets:
                    ...
                    eval_url = f"{self.debug_url}/json/runtime/evaluate"
                    
                    # Send location.reload() command
            ...
            # Method 2: Try page reload via direct URL access
            try:
                reload_url = f"{self.debug_url}/json/page/reload"
                response = urllib.request.urlopen(reload_url, timeout=3)
                return response.status == 200
            except:
                pass
    ...
    def hard_restart(self) -> bool:
        """Kill and restart Premiere Pro entirely"""
        try:
            import psutil
            
            # Find and kill Premiere processes
            for proc in psutil.process_iter(['pid', 'name']):
                if 'Adobe Premiere Pro' in proc.info['name']:
                    proc.terminate()
                    proc.wait(timeout=5)
...
Examples:
    ...
    if command == 'register':
        ...
        if len(sys.argv) < 3:
            print("❌ Usage: restart <plugin-id|all> [--hard]")
            return
        
        target = sys.argv[2]
        hard_restart = '--hard' in sys.argv
        
        if target == 'all':
            print("🔄 Restarting all registered plugins...")
            success_count = 0
            for plugin_id, config in controller.plugins.items():
                restarter = PluginRestarter(config.debug_port, config.panel_name)
                if hard_restart:
                    print(f"🔄 Hard restarting {plugin_id}...")
                    if restarter.hard_restart():
                        print(f"✅ {plugin_id} restarted successfully")
                        success_count += 1
                    else:
                        print(f"❌ {plugin_id} restart failed")
                else:
                    print(f"🔄 Soft restarting {plugin_id}...")
                    if restarter.soft_restart():
                        print(f"✅ {plugin_id} restarted successfully")
                        success_count += 1
                    else:
                        print(f"⚠️ {plugin_id} soft restart failed, trying hard restart...")
                        if restarter.hard_restart():
                            print(f"✅ {plugin_id} hard restarted successfully")
                            success_count += 1
                        else:
                            print(f"❌ {plugin_id} restart failed completely")
            
            print(f"\n🎉 Restarted {success_count}/{len(controller.plugins)} plugins successfully")
        else:
            if target not in controller.plugins:
                print(f"❌ Plugin '{target}' not registered")
                return
            
            config = controller.plugins[target]
            restarter = PluginRestarter(config.debug_port, config.panel_name)
            
            if hard_restart:
                print(f"🔄 Hard restarting {target}...")
                if restarter.hard_restart():
                    print(f"✅ {target} restarted successfully")
                else:
                    print(f"❌ {target} restart failed")
            else:
                print(f"🔄 Soft restarting {target}...")
                if restarter.soft_restart():
                    print(f"✅ {target} restarted successfully")
                else:
                    print(f"⚠️ Soft restart failed, trying hard restart...")
                    if restarter.hard_restart():
                        print(f"✅ {target} hard restarted successfully")
                    else:
                        print(f"❌ {target} restart failed completely")
