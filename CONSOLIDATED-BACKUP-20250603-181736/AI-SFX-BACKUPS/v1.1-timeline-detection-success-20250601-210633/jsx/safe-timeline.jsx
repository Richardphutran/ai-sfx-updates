...
function importAndPlaceAudioAtTime(filePath, trackIndex, timeSeconds) {
    ...
    try {
        ...
        // Step 4: Create time object for specific placement
        result.step = "creating_time_object";
        var placementTime;
        if (typeof sequence.createTime === 'function') {
            placementTime = sequence.createTime();
            placementTime.seconds = timeSeconds;
        } else {
            // Fallback: use time object structure
            placementTime = {
                seconds: timeSeconds,
                ticks: Math.floor(timeSeconds * sequence.timebase)
            };
        }
